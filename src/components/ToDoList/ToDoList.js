...
class ToDoList extends Component {
  ...
  initializeContract = async () => {
    ...
    if (accounts.length === 0) accounts = await window.ethereum.enable();

    if (accounts.length > 0) {
      console.log("account: ", accounts[0]);
      this.setState({ account: accounts[0] });
    } else window.alert("Could not read accounts from network");
        ...
        <div className="flex-container" style={{ justifyContent: "center" }}>
          ...
          <div style={{ flexGrow: "2" }}>
            <ListItem
              ...
              deleteItem={(e, id) => this.deleteItem(e, id)}
            />
          </div>
          <div style={{ flexGrow: "3" }}></div>
        </div>
        ...
        <div key={listItem.id}>
          ...
          >
            ...
            <span
              ...
              onClick={e => props.deleteItem(e, listItem.id)}
            ></span>
          </div>
        </div>
      ))}
